...
(defn only-anagramic
  "take a string word and a set of tuples of two words, 
   returns a list of only the tuples that combined into a single string
   are anagrams of the first word"
  [word set-combs]
  (set (filter #(is-anagram? word (reduce str %)) set-combs)))
